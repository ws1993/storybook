...
export async function configToCsfFactory(
  ...
  configType = configType || (info.path.includes('preview') ? 'preview' : 'main');
...
export const csfFactories: CommandFix = {
  id: 'csf-factories',
  promptType: 'command',
  async run({ dryRun, mainConfig, mainConfigPath, previewConfigPath, packageJson }) {
